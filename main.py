...
if args.serve:
    print('ok, tu')
    path = args.serve
    coro = loop.create_server(lambda *args, **kwargs: ServerProtocol(path, *args, **kwargs), '127.0.0.1', port)
    ...
    print('Downloading...')
    loop = asyncio.get_event_loop()
    coro = loop.create_connection(lambda: ClientProtocol(args), '127.0.0.1', port)
